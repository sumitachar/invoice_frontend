...
function IllustrationLayout({ header, title, description, illustration, children }) {
  ...
  const { darkMode } = controller || {}; // Ensure controller is not undefined

  console.log("Illustration Layout Props:", { header, title, description, illustration });
      ...
      <Grid
        ...
        sx={{
          backgroundColor: ({ palette: { background, white } }) =>
            darkMode ? background?.default || "#000" : white?.main || "#fff",
        }}
      >
        <Grid item xs={12} lg={6}>
          <MDBox
            ...
            ml={2}
            mt={2}
            sx={{ backgroundImage: illustration ? `url(${illustration})` : "none" }}
          />
        </Grid>
